"""
Fine-tune Moore-AnimateAnyone Stage 2 (motion/temporal module) from stage 1 fine-tuned weights.
Trains: only motion_modules inside denoising_unet
Frozen: everything else (vae, image_encoder, reference_unet, pose_guider, spatial layers)

Loads stage 1 fine-tuned reference_unet, denoising_unet, pose_guider, 
plus the pretrained motion_module.
"""
...
from animate_anyone.pipelines.pipeline_pose2vid import Pose2VideoPipeline
from animate_anyone.utils.util import (
...
def find_latest_stage1_step(stage1_dir):
    """Find the latest checkpoint step from stage 1 output."""
    files = os.listdir(stage1_dir)
    ref_files = [f for f in files if f.startswith("reference_unet-") and f.endswith(".pth")]
    if not ref_files:
        raise FileNotFoundError(f"No stage 1 checkpoints found in {stage1_dir}")
    steps = [int(f.split("-")[1].split(".")[0]) for f in ref_files]
    return max(steps)
...
def main(cfg):
    ...
    # Initialize reference_unet from SD1.5 architecture
    reference_unet = UNet2DConditionModel.from_pretrained(
        cfg.base_model_path,
        subfolder="unet",
    ).to(device="cuda", dtype=weight_dtype)

    # Initialize denoising_unet with motion modules
    ...
    stage1_step = find_latest_stage1_step(stage1_ckpt_dir)
    logger.info(f"Loading stage 1 checkpoint at step {stage1_step}")

    denoising_unet.load_state_dict(
        torch.load(
            os.path.join(stage1_ckpt_dir, f"denoising_unet-{stage1_step}.pth"),
            map_location="cpu",
        ),
        strict=False,
    )
    reference_unet.load_state_dict(
        torch.load(
            os.path.join(stage1_ckpt_dir, f"reference_unet-{stage1_step}.pth"),
            map_location="cpu",
        ),
        strict=False,
    )
    pose_guider.load_state_dict(
        torch.load(
            os.path.join(stage1_ckpt_dir, f"pose_guider-{stage1_step}.pth"),
            map_location="cpu",
        ),
        strict=False,
    )
    ...
    logger.info(f"Total trainable params (motion modules): {len(trainable_params)}")
    ...
    logger.info("***** Running fine-tuning (Stage 2 - motion module) *****")
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                pixel_values_pose = pixel_values_pose.transpose(1, 2)

                uncond_fwd = random.random() < cfg.uncond_ratio
                clip_image_list = []
                ref_image_list = []
                for batch_idx, (ref_img, clip_img) in enumerate(
                    zip(batch["pixel_values_ref_img"], batch["clip_ref_img"])
                ):
                    if uncond_fwd:
...
def save_motion_module(model, save_dir, prefix, ckpt_num, total_limit=None):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/train/finetune_stage2.yaml")
